...
public class BookService {
    ...
    /*
     * Metodo per decrementare le quantità dei libri acquistati sul microservizio Sales
     */
    public void editBookQuantity(List<BookQuantityRequest> bookQuantityRequestList) {
        // Iteriamo i libri acquistati
        for (BookQuantityRequest bookQuantityRequest: bookQuantityRequestList) {
            // Recuperiamo ogni libro
            Optional<BookEntity> bookEntityOpt = bookRepository.findById(bookQuantityRequest.getBookId());
            if (bookEntityOpt.isEmpty()) {
                throw new NotFoundException("Libro non presente in catalogo");
            }
            // Recupera la vecchia quantità e ne decrementa, in base al numro di vendite e salva la nuova quantità.
            Integer oldQuantity = bookEntityOpt.get().getQuantity();
            Integer newQuantity = oldQuantity - bookQuantityRequest.getQuantity();
            bookEntityOpt.get().setQuantity(newQuantity);
